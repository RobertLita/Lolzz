...
async function getSummonerDetails(
  summonerName: string,
  region: string
): Promise<SummonerDetails | string> {
  try {
...
async function getMatchDetails(
  ...
  try {
    ...
    objectives.noBarons =
      data["info"]["teams"][team]["objectives"]["baron"]["kills"];
    objectives.noDragons =
      data["info"]["teams"][team]["objectives"]["dragon"]["kills"];
    objectives.noTurrets =
      data["info"]["teams"][team]["objectives"]["tower"]["kills"];
...
export async function getSummonerHistory(
  summonerName: string,
  region: string
): Promise<Info | string> {
  let summonerInfo: SummonerDetails | string = await getSummonerDetails(
    summonerName,
    region
  );
  ...
  let matchIds: string[] | string = await getMatchList(
    summonerInfo.puuid,
    region,
    0
  );
  ...
  for (let i = 0; i < matchIds.length; i++) {
    let match: Match | string = await getMatchDetails(
      summonerName,
      matchIds[i],
      region
    );
